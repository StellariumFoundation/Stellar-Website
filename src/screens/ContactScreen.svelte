<script lang="ts">
  ...
  $effect(() => {
    ...
    const unsub4 = callClient.queuePosition.subscribe((v) => queuePosition = v);
    return () => {
      unsub1();
      unsub2();
      unsub3();
      ...
      {:else if callState === 'calling'}
        <div class="flex flex-col items-center gap-2 py-3">
          {#if queuePosition !== null}
            <span class="text-amber-300 text-sm">In Queue — Position #{queuePosition}</span>
            <span class="text-gray-400 text-xs">Waiting for callee to become available...</span>
          {:else}
          ...
          <button
            ...
            class="mt-2 text-xs text-red-400 hover:text-red-300 uppercase tracking-wider transition-colors"
          >Cancel</button>
        </div>
