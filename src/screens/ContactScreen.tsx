...
export function ContactScreen() {
  ...
  return (
    <div className="flex flex-col h-full overflow-y-auto w-full p-6 items-center">
      <div className="mb-4">
        <Shield size={48} className="text-[var(--color-primary)] opacity-80" />
      </div>
      <h1 className="text-2xl text-[var(--color-primary)] font-bold text-center mt-2 uppercase tracking-wide">
        Secure Comms
      </h1>
      <p className="text-center text-sm text-gray-400 mt-4 max-w-sm leading-relaxed mb-8">
      ...
      <div className="w-full max-w-md space-y-4 pb-24">
         <div className="bg-[#1E1E1E] border border-white/10 rounded-2xl p-6">
            ...
            <input 
              ...
              className="w-full bg-black/40 border border-white/5 rounded-xl p-4 text-white focus:outline-none focus:border-[var(--color-primary)] transition-colors placeholder:text-gray-600"
              placeholder="Leave empty for anonymity"
              disabled={isSending}
            />

            <label className="block text-xs font-bold text-gray-500 uppercase tracking-widest mt-6 mb-2">
              Intel / Message
            </label>
            <textarea 
              value={message}
              ...
              rows={6}
              className="w-full bg-black/40 border border-white/5 rounded-xl p-4 text-white focus:outline-none focus:border-[var(--color-primary)] transition-colors resize-none placeholder:text-gray-600"
            ...
            <button 
              ...
              className="w-full mt-6 bg-[var(--color-primary)] text-black font-bold py-4 rounded-xl flex items-center justify-center gap-2 uppercase tracking-widest text-sm hover:bg-[var(--color-primary)]/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all"
            >
              {isSending ? (
                <>Encrypting...</>
              ) : (
