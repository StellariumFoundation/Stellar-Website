<script lang="ts">
  import { marked } from 'marked';
   import { ArrowLeft, X, Send, MessageCircle, Copy, PlaySquare, Droplet, ExternalLink, Shield, CheckCircle, Globe, Lightbulb, Briefcase, Users, TrendingUp, Brain, Eye, Download, AlertTriangle } from '@lucide/svelte';
...
{:else}
  <div class="flex flex-col h-full overflow-y-auto no-scrollbar w-full max-w-5xl lg:max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 items-center">
    <img src="/icons/icon-512.png" alt="Stellarium" class="w-16 h-16 lg:w-20 lg:h-20 mt-6 lg:mt-10" />
    ...
    <!-- Operation Radiohead Critical Alert -->
    <div class="mt-10 lg:mt-16 w-full max-w-md lg:max-w-xl xl:max-w-2xl flex flex-col items-center border-t border-white/10 pt-8">
      <h3 class="text-xs lg:text-sm xl:text-base uppercase tracking-[0.2em] text-red-400 mb-4 flex items-center gap-2">
        <AlertTriangle size={14} /> Critical Alert
      </h3>
      <div class="w-full bg-gradient-to-b from-[#1f0a0a] to-[#0c0505] border border-red-500/30 hover:border-red-500/50 rounded-2xl p-5 shadow-[0_8px_40px_rgba(255,0,0,0.12)] transition-all duration-300 relative overflow-hidden group">
        <div class="absolute top-0 right-0 w-24 h-24 bg-red-500/10 rounded-full blur-2xl group-hover:bg-red-500/20 transition-all duration-300 pointer-events-none" />
        <div class="relative z-10">
          <div class="flex items-center justify-between gap-2 mb-3">
            <div class="flex items-center gap-2">
              <div class="p-2 bg-red-500/15 text-red-400 border border-red-500/25 rounded-full shrink-0">
                <Brain size={18} class="animate-pulse" />
              </div>
              <h4 class="text-sm text-white tracking-wide">OPERATION RADIOHEAD</h4>
            </div>
            <span class="inline-block px-2 py-0.5 bg-red-500/15 text-red-400 border border-red-500/30 text-[9px] rounded-full uppercase tracking-wider animate-pulse">Live Dossier</span>
          </div>

          <p class="text-xs text-gray-300 leading-relaxed mb-4">
            <span class="text-red-400 font-medium">RADIOHEAD RECORD — 8 Billion Node Transmission.</span>
            A coordinated technological hostage event turned the global neural implant network into the largest-scale atrocity broadcasting platform in human history.
          </p>

          <div class="grid grid-cols-2 gap-2 mb-4">
            <div class="bg-black/30 border border-white/5 rounded-xl p-2.5 text-center">
              <div class="text-[8px] text-gray-500 uppercase tracking-wider mb-0.5">Attack Date</div>
              <div class="text-xs text-white">20 Jun 2024</div>
            </div>
            <div class="bg-black/30 border border-white/5 rounded-xl p-2.5 text-center">
              <div class="text-[8px] text-gray-500 uppercase tracking-wider mb-0.5">Direct Victims</div>
              <div class="text-xs text-red-400">2,100+</div>
            </div>
            <div class="bg-black/30 border border-white/5 rounded-xl p-2.5 text-center">
              <div class="text-[8px] text-gray-500 uppercase tracking-wider mb-0.5">Affected</div>
              <div class="text-xs text-white">~8 Billion</div>
            </div>
            <div class="bg-black/30 border border-white/5 rounded-xl p-2.5 text-center">
              <div class="text-[8px] text-gray-500 uppercase tracking-wider mb-0.5">Status</div>
              <div class="text-xs text-red-400 flex items-center justify-center gap-1">
                <span class="w-1.5 h-1.5 rounded-full bg-red-500 animate-pulse" /> ACTIVE
              </div>
            </div>
          </div>

          <a
            href="https://operationradioheaddossier.onrender.com/"
            target="_blank"
            rel="noopener noreferrer"
            class="inline-flex items-center justify-center gap-2 w-full px-5 py-3 bg-red-600 hover:bg-red-700 text-white rounded-xl text-xs uppercase tracking-wider shadow-lg hover:shadow-red-500/20 transition-all duration-300 group/btn"
          >
