...
<div class="flex flex-col h-[100dvh] w-full bg-transparent">
  ...
  <div class="bg-black/70 backdrop-blur-md border-t border-white/5 safe-bottom z-50">
    <div class="flex justify-center items-center h-16 md:h-20 lg:h-24 px-2 gap-1 sm:gap-2 md:gap-4 lg:gap-8 max-w-lg md:max-w-3xl lg:max-w-5xl mx-auto">
      {#each tabs as { path, icon: Icon, label }}
        <button
          onclick={() => navigate(path)}
          class="flex flex-col items-center justify-center w-12 sm:w-14 md:w-16 lg:w-20 h-full space-y-1 transition-colors relative {matchRoute(path) ? 'text-[var(--color-tertiary)]' : 'text-gray-400 hover:text-white'}"
        >
