...
- **Two Android Gradle projects**: `android/` (Capacitor, AGP 9.2.1, Gradle 9.4.1, JDK 25) and `StellariumCaller/` (native, AGP 9.2.1, Gradle 9.4.1, JDK 25). Capacitor has `android.builtInKotlin=false` + `android.newDsl=false` in `gradle.properties` for AGP 9.x compatibility — remove when plugins support AGP 10.
